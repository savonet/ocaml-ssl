...
CAMLprim value ocaml_ssl_get_version() {
  ...
  Store_field(result, 0, Val_int(OPENSSL_VERSION_MAJOR));
  Store_field(result, 1, Val_int(OPENSSL_VERSION_MINOR));
  Store_field(result, 2, Val_int(OPENSSL_VERSION_PATCH));
#else
...
CAMLprim value ocaml_ssl_get_client_verify_callback_ptr(value unit) {
  ...
  if (Is_long(vclient_verify_callback)) {
    ...
    *((int (**)(int, X509_STORE_CTX *))Data_abstract_val(
        vclient_verify_callback)) = client_verify_callback;
...
CAMLprim value ocaml_ssl_ctx_set_verify(value context, value vmode,
                                        value vcallback) {
...
CAMLprim value ocaml_ssl_ctx_set_client_CA_list_from_file(value context,
                                                          value vfilename) {
...
CAMLprim value ocaml_ssl_ctx_set_cipher_list(value context,
...
CAMLprim value ocaml_ssl_get_current_cipher(value socket) {
  ...
  if (!cipher)
    caml_raise_constant(*caml_named_value("ssl_exn_cipher_error"));
#if defined(NO_NAKED_POINTERS) || defined(NAKED_POINTERS_CHECKER)
  value vcipher = caml_alloc(1, Abstract_tag);
...
CAMLprim value ocaml_ssl_verify(value socket) {
  ...
  if (ans != 0) {
    if (2 <= ans && ans <= 32)
      caml_raise_with_arg(*caml_named_value("ssl_exn_verify_error"),
...
CAMLprim value ocaml_ssl_write_bigarray(value socket, value buffer, value start,
                                        value length) {
  CAMLparam2(socket, buffer);
  int ret, err;
  SSL *ssl = SSL_val(socket);
...
CAMLprim value ocaml_ssl_read(value socket, value buffer, value start,
                              value length) {
  CAMLparam2(socket, buffer);
  int ret, err;
  int buflen = Int_val(length);
...
CAMLprim value ocaml_ssl_read_into_bigarray(value socket, value buffer,
                                            value start, value length) {
  CAMLparam2(socket, buffer);
  int ret, err;
  struct caml_ba_array *ba = Caml_ba_array_val(buffer);
...
CAMLprim value ocaml_ssl_read_into_bigarray_blocking(value socket, value buffer,
                                                     value start,
                                                     value length) {
  CAMLparam2(socket, buffer);
  int ret;
...
#define VERIFY_ERR_DEPTH_ZERO_SELF_SIGNED_CERT                                 \
...
static int client_verify_callback(int ok, X509_STORE_CTX *ctx) {
  ...
  if (error == VERIFY_ERR_DEPTH_ZERO_SELF_SIGNED_CERT) {
    if (1) {
       ...
       */
      if (client_verify_callback_verbose) {
        fprintf(stderr, "SSL: rejecting connection - server has a self-signed "
                        "certificate\n");
        fflush(stderr);
      }
  ...
  if (!((error == VERIFY_OK) || (error == VERIFY_ROOT_OK))) {
    if (1) {
       ...
       */
      if (error == VERIFY_ERR_UNABLE_TO_GET_ISSUER &&
          client_verify_callback_verbose) {
